...
def write_page_addr(conn, ad, page_addr):
    if(ad == "analog"):
        #spi_command(conn, "write_adc_spi 0x%x 0x%x 0x%x\n" % (0x0, 0x11, page_addr))
        adc_spi(conn, 0x0, 0x11, page_addr)
    else:
        page_addr1 = page_addr & 0xFF
        page_addr2 = (page_addr & 0xFF00) >> 8
        adc_spi(conn, 0x4, 0x3, page_addr1)
        adc_spi(conn, 0x4, 0x4, page_addr2)
        #spi_command(conn, "write_adc_spi 0x%x 0x%x 0x%x\n" % (0x4, 0x3, page_addr1))
        #spi_command(conn, "write_adc_spi 0x%x 0x%x 0x%x\n" % (0x4, 0x4, page_addr2))

def read_reg(conn, ad, reg):
    # This functions assumes the page address has been written to the
    # page selection address
    values = None
    if(ad == "analog"):
        values = adc_spi(conn, 0x8, reg, 0x0)
        #values = spi_command(conn, "write_adc_spi 0x%x 0x%x 0x%x\n" % (0x8, reg, 0x0))
    else:
        values = adc_spi(conn, 0xE, reg, 0x0)
        #values = spi_command(conn, "write_adc_spi 0x%x 0x%x 0x%x\n" % (0xE, reg, 0x0))
    return values[-1]

def adc_readback(conn):
    for ad, pages in adc_regs.items():
        for page_name, page_info in  pages.items():
            page_addr = page_info['page_addr']
            regs = page_info['regs']
            write_page_addr(conn, ad, page_addr)
            for reg in regs:
                v = read_reg(conn, ad, reg)
